...
pip install "gracenote2epg @ git+https://github.com/th0ma7/gracenote2epg.git@v1.6.1"
```
...
wget https://github.com/th0ma7/gracenote2epg/archive/v1.6.1.tar.gz -O gracenote2epg-1.6.1.tar.gz

# Install into /usr/local/
sudo tar -xzf gracenote2epg-1.6.1.tar.gz -C /usr/local/

# Create a generic gracenote2epg symbolic link
sudo ln -sf /usr/local/gracenote2epg-1.6.1 /usr/local/gracenote2epg
...
sudo su -s /bin/bash sc-tvheadend -c '/var/packages/tvheadend/target/env/bin/pip3 install "gracenote2epg[full]==1.6.1"'
...
pip list | grep gracenote2epg
# Expected output: gracenote2epg    1.6.1
